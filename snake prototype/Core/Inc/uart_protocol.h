 ...
 * v1.6 (uart_protocol.md).
 *
 * CHANGES in v1.6:
 * - UART_SendScore() now takes snake_len parameter
 * - CMD_SCORE payload updated: [snake_len][score_lsb][score_msb]
 *
...
#define CMD_SCORE       0x83    /**< Current score and snake length */
...
/*---------------------------------------------------------------------------*
 * Payload structure for CMD_SCORE (0x83)                                    *
 *---------------------------------------------------------------------------*/
/**
 * @brief Score payload format (3 bytes used, 2 unused):
 * - Byte 0: Snake length (1-100)
 * - Byte 1: Score LSB (low byte)
 * - Byte 2: Score MSB (high byte)
 * - Bytes 3-4: Unused (0x00)
 *
 * Example: snake_len=15, score=300 (0x012C)
 * Payload: [0x0F, 0x2C, 0x01, 0x00, 0x00]
 */
/* (No separate structure needed, just documentation) */
 ...
 *
 * @note Packet payload format (Little-Endian):
 *       payload[0] = snake_len
 *       payload[1] = score & 0xFF (LSB)
 *       payload[2] = (score >> 8) & 0xFF (MSB)
 */
